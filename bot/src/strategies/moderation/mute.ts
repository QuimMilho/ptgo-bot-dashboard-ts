...
export async function mute(
	...
	if (member) {
		...
		const info: BDPunitions[] = await client.query(
			'SELECT * FROM PUNITIONS WHERE CLIENTID LIKE ? AND EXPIRED = FALSE',
			[user.id]
		);
		if (info.length != 0) {
			return 'AlreadyMuted';
	...
	await client.query(
		'INSERT INTO PUNITIONS (GUILDID, CLIENTID, ADMINID, TIMEDONE, EXPIRES, EXPIRED, REASON, TIPO)' +
			` VALUES (?, ?, ?, NOW(), NOW(), ?, ?, 'mute')`,
...
export async function tempMute(
	...
	await client.query(
		'INSERT INTO PUNITIONS (GUILDID, CLIENTID, ADMINID, TIMEDONE, EXPIRES, EXPIRED, REASON, TIPO)' +
			` VALUES (?, ?, ?, NOW(), NOW() + interval ? minutes, ?, ?, 'mute')`,
